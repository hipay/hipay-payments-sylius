 ...
 */
final class MultibancoPaymentMethodProcessorPayment implements PaymentOrderRequestProcessorInterface
{
    public function process(OrderRequest $orderRequest, PaymentOrderRequestContext $context): void
    {
        $orderRequest->payment_product = PluginPaymentProduct::MULTIBANCO->value;
        /** @var array<string, mixed> $configuration */
        $configuration = $context->gatewayConfig['configuration'] ?? $context->gatewayConfig;
        // @phpstan-ignore-next-line
        $orderRequest->expiration_limit = $configuration['expiration_limit'] ?? '1';
    }
}
